...
# Config 
class Config(object):
    BOT_TOKEN = os.environ.get("BOT_TOKEN", "7959246531:AAFAf0zP8aYwoTYbFvfgh16R8ctj9SKMWi8")
    API_ID = int(os.environ.get("API_ID",  "25749031"))
    API_HASH = os.environ.get("API_HASH", "7d11aa227ccb141e1fdd8506971bd031")
    DOWNLOAD_LOCATION = "./DOWNLOADS"
    SESSIONS = "./SESSIONS"

    AUTH_USERS = os.environ.get('AUTH_USERS', '1898405489').split(',')
    for i in range(len(AUTH_USERS)):
        AUTH_USERS[i] = int(AUTH_USERS[i])

    GROUPS = os.environ.get('GROUPS', '-1002693626661').split(',')
    for i in range(len(GROUPS)):
        GROUPS[i] = int(GROUPS[i])

    LOG_CH = os.environ.get("LOG_CH", "-1002660594801")
...
if __name__ == "__main__":
    ...
    async def main():
        await PRO.start()
        # h = await PRO.get_chat_member(chat_id= int(-1002693626661), user_id=1898405489)
        # print(h)
