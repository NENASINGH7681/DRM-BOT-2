        ...
        try:
            ...
            for line in lines:
                ...
                for url in urls:
                    ...
                    if not name_part:
                        name_part = f"File_{str(count).zfill(3)}"
                    name_links.append((name_part.strip(), url.strip()))
                    count += 1
            if not name_links:
                ...
                return []
            os.remove(x)
            print(f"Parsed {len(name_links)} links successfully.")
            return name_links
        except Exception as e:
            LOGS.error(str(e))
            await self.m.reply_text("**Error processing file.**")
            os.remove(x)
            return []
    def parse_name(self, rawName):
        name = (
            rawName.replace("/", "_")
            .replace("|", "_")
            .replace(":", "-")
